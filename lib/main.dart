import 'package:blocwithsearch/search_bloc.dart';
  ...
  Widget build(BuildContext context) {
    return RepositoryProvider(
      ...
      child: MultiBlocProvider(
          providers: [
            BlocProvider(
                create: (context) => SearchBloc(searchUserRepository: RepositoryProvider.of<SearchUserRepository>(context))
            ),
          ],
          child: MaterialApp(
            theme: ThemeData(
...
class UserInfoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(user.username ?? '',style: TextStyle(fontSize: 22),),
      ),
      body: Column(
        children: [
          Hero(tag: user.username ?? '', child: Container(
